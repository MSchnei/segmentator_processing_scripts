...
for (( k=0; k<${switchLen}; k++ )); do
	...
	for (( j=0; j<${subjLen}; j++ )); do
	  ...
	  truth="${parentpath}/${subj}/derived/01_ground_truth/${subj}_gm_0?_borders.nii.gz"
	  # loop throuh different segmentation results
	  for (( i=0; i<${tLen}; i++ )); do
	    # deduce name for segmentation result file
	    segm="${parentpath}/${subj}/derived/05_gm/${subj}_division_${switch}_gm${res[i]}_borders.nii.gz"
	    # evaluate the segmentation
	    command="${evalseg} ${truth} ${segm} "
	    command+="-use AVGDIST "
	    command+="-xml ${parentpath}/${subj}/derived/07_evaluation/${switch}${res[i]}_dst.xml"
	    echo "${command}"
	    ${command}
	  done
	done
