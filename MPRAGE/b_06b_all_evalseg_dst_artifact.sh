	...
	for (( j=0; j<${subjLen}; j++ )); do
	  ...
	  for (( i=0; i<${tLen}; i++ )); do
	    # deduce name for segmentation result file
			segm="${parent_path}/${subj}/derived/06_gm_artifact_masked/${subj}_division_${switch}_gm${res[i]}_borders_artifact_masked.nii.gz"
	    # evaluate the segmentation
	    command="${evalseg} ${truth} ${segm} "
	    command+="-use AVGDIST "
			command+="-xml ${parent_path}/${subj}/derived/08_evaluation_artifact_masked/${switch}${res[i]}_artifact_masked_dst.xml"
	    echo "${command}"
	    ${command}
	  done
	done
