function [ B1temp T1temp MP2RAGEcorrected] = T1B1correctpackage( Sa2RAGEimg,B1,Sa2RAGE,MP2RAGEimg,T1,MP2RAGE,brain,varargin)
...
%
% MP2RAGE.TR=6;
% MP2RAGE.TRFLASH=6.2e-3;
% MP2RAGE.TIs=[800e-3 2500e-3];
% MP2RAGE.NZslices=[ 80 80];%excitations before and after kspace centre
% MP2RAGE.FlipDegrees=[4 5];
% MP2RAGE.averageB1=1;
%
...
set(gcf,'Color',[1 1 1]);
hold off
for B_one=0.6:0.2:1.4
    [MP2RAGEamp T1vector IntensityBeforeComb]=MP2RAGE_lookuptable(2,MP2RAGE.TR,MP2RAGE.TIs,B_one*MP2RAGE.FlipDegrees,MP2RAGE.NZslices,MP2RAGE.TRFLASH,'normal');
    plot(MP2RAGEamp,T1vector,'color',[0.5 0.5 0.5]*B_one,'Linewidth',2)
    hold on
end
...
if isempty(MP2RAGEimg)
    ...
    MP2RAGEimg.img=reshape(interp1(MP2RAGE.T1vector,MP2RAGE.Intensity,T1.img(:)),size(B1.img));
...
if isempty(Sa2RAGEimg)
    ...
    Sa2RAGEimg.img=reshape(interp1(Sa2RAGE.B1vector,Sa2RAGE.Intensity,B1.img(:)),size(B1.img));
else
...
figure(4); imagesc(Sa2RAGEmatrix);
figure(5); imagesc(MP2RAGEmatrix); 
...
assignin('base','B1vector',B1temp.img);
